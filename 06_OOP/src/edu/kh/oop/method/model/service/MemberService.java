...
public class MemberService {
	...
	public String signUp() {
		...
		if(memberPw.equals(memberPw2)) {//일치하는 경우
			...
			// 생성된 객체의 주소를 필드에 있는 memberInfor에 대입
			
			memberInfo = new Member(memberId,memberPw,memberName,memberAge);
			//memberPw 또는 memberPw2가 일치한다는 조건을 주었기에 둘중 하나 아무거나 넣어도 상관없음!
			...
			return "회원가입 실패 (비밀번호 불일치!)"; // == return null;
		}
	...
	public String login() {
		...
		// 저장된 아이디, 비밀번호가 입력받은 아이디, 비밀번호와 같으면 "로그인 성공"
		// 같지 않으면 "아이디 또는 비밀번호가 일치하지 않습니다" 리턴
		
		// 아이디랑 비밀번호가 모두 일치할 경우
		if(memberId.equals(memberInfo.getMemberId()) && 
				memberPw.equals(memberInfo.getMemberPw()) ) {
			// 입력받은 memberId와 memberInfo 필드에서 참조중인 Member 객체의 memberId와 같은지 확인
			// 입력받은 memberPw와 memberInfo 필드에서 참조중인 Member 객체의 memberPw와 같은지 확인
		
			// loginMember에 Member 객체 주소 대입
			loginMember = memberInfo; 
		  // 참조형변수 = Member 객체 주소(얕은 복사) => 주소만 복사하는 것 !
			
			// 회원가입 정보를 loginMember에 대입하여 어떤 회원이 로그인했는지를 구분할 수 있게 함.
	...
	public int updateMember() { //int형으로 작성했지만 위에 String 형 처럼 return값에 문자열로 작성해도 상관없음!
								//다른 방법이 있음을 보여주기 위한것일뿐@
