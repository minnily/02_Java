...
//[접근 제한자][예약어 (현재는 예약어 없는 상태)] class 클래스명
	
	//접근제한자 public : 같은 프로젝트 내에서 어떤 클래스(==어떤 패키지 _즉, 전체)에서든 import 할 수 있음을 나타냄.
	
	
	//1.필드(field) : 객체의 속성을 작성하는 클래스 내부 영역 (==멤버변수 : 메서드 밖에 작성된 변수)
	...
	public static String schoolName ="KH고등학교"; //프로그램이 시작되자마자 static 영역에 메모리가 할당(JVM읽음)
	...
	public Student() {
		//이 곳에 객체가 생성될 때 바로 수행할 코드를 작성한다.
