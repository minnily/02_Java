...
public class CharacterService {
	...
	public void fileOutput() {
		...
		sb.append("그리고 이거 끝나고 쪽지 시험입니다.\n");
		
		String content = sb.toString();
		
		// content에 저장된 문자열을 
		///io_test/202400222/출력테스트.txt 파일로 출력하기
		...
		try {
			...
			// 파일과 출력 스트림 연결 (파일이 없으면 자동으로 생성함)
			...
			//bw.flush(); =>close()에서 자동으로 되서 적지 않아도 됨.
			...
			try {
				// close() 구문에 flush() 가 내장되어 있어서 쓰지 않아도 된다!
				if(bw != null) bw.close();
			} catch (IOException e) {
				e.printStackTrace();
			}
