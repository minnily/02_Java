...
public class MemberServicePractice1 {
	...
	public MemberServicePractice1() {//기본생성자
		
		//memberArr 배열 0,1,2 인덱스 초기화
		memberArr[0]= new Member("user01","pss01", "홍길동", 10, "서울");
		memberArr[1]= new Member("user02","pss02", "김영희", 20, "경기");
		memberArr[2]= new Member("user03","pss03", "박철수", 30, "강원");
	...
	public void displayMenu() {
		...
		do {
			...
			default : System.out.println("잘못 입력하셨습니다. 다시 입력 하세요");
			}
	...
	public int emptyIndex() {
		...
		//memberArr 배열을 0 인덱스부터 배열의 끝까지 접근해서
		// 참조하는 값이 null 인 경우의 인덱스를 반환
		
		for(int i =0; i <memberArr.length;i++) {
			if(memberArr[i] == null) {
				return i; //현재 메서드를 종료하고 호출한 곳으로 i 값을 가지고 돌아간다는 의미
							//  i 는 인덱스 번호이다!
			}
				
		}
		
		//for 문을 수행했지만 return 이 되지 않은 경우 해당 위치 코드가 수행된다.
		//-> for 문에서 return 이 되지 않았다는 것은 배열에 빈칸이 없다는 의미와 같다.
		return -1;
	...
	public String signUp() {
		...
		//-> 새로운 회원정보를 저장할 공간이 있는지 확인하고 빈 공간의 인덱스 번호 얻어오기!
		...
		if(memberPw.equals(memberPw2)) {//일치하는 경우
			...
			//Member 객체 생성해서 할당된 주소를 memberArr의 비어있는 인덱스 배열에 대입해야함
	...
	public String login() {
		...
		for(int i=0; i< memberArr.length; i++) {
			...
			if(memberArr[i] != null) {//회원 정보가 있을 경우 (비어있는 인덱스가 아닌 경우)
				
				// 2) 저장되어 있는 회원정보(memberArr[i])의 아이디, 비밀번호와 
				...
				if(memberArr[i].getMemberId().equals(memberId) &&
					...
					//현재 접근중인 memberArr[i] 요소에 저장된 주소를 얕은 복사 할 것임.
					loginMember = memberArr[i]; 
					break; //더이상 같은 아이디, 비밀번호가 없기 때문에 가장 위에 있는 for문 종료시키는 것
	...
	//6) 로그아웃 메서드 
	public void logout() {
		if(loginMember != null) {// 1) 로그인이 되어 있는 경우
			loginMember = null; // 2) null 값을 넣어줌
			System.out.println("로그아웃 하였습니다");
		} else { //3) 로그인이 되어있지 않은 경우
			System.out.println("로그인 안했는데용?");
		}
	}
	...
	public void searchRegion() {
		...
		for(int i =0; i < memberArr.length; i++) {
			...
			// 3) memberArr[i] 요소에 저장된 지역(region)이 입력받은 지역(inputregion)이 같을 경우
			//		 회원의 아이디, 이름을 출력!
