...
public class PolyService {
	...
	public void ex1() {
		...
		// 객체 배열 : 같은 객체 참조 자료형의 변수를 하나의 묶음으로 다루는 것
					...
					//다형성을 이용하면 부모 타입 참조 자료형의 변수를 하나의 묶음으로 다룰 수 있음
	...
	//전달 받은 Car 또는 자식 객체(Tesla,Spark)의 엔진, 연료, 바퀴 갯수를 출력하는 메서드!
	...
	public void ex2() {
		...
		printCar(s); //Spark
		printCar(c);// Car
		//가능한 이유 : car타입 빼고  모두 상속받고 있기에 업캐스팅을 한 것이다.
		...
		System.out.println(arr[1] instanceof Car); //true 부모객체가Car이기 때문!
	...
	public void ex3() {
		...
		//다운 캐스팅이란?
		//부모 타입 참조변수가 자식 객체를 참조하는 기술로 업캐스팅 상태에서만 진행할 수 있다.
		//부모 타입을 자식타입으로 "강제 형변환"해서 자식객체의 본래 필드, 메서드를 사용가능하게 한다.
		
		
		Car c1 = new Tesla("전기모터", "전기", 4, 50000);
		//업 캐스팅 상태
		
		System.out.println("다운캐스팅 방법 : "+((Tesla)c1).getBatteryCapacity());
		//주의사항! "." 연산자가 (Tesla) 형변환 연산자보다 우선순위가 높음 그래서 ()한번더 감싸줘야한다!
		
		Tesla t1 =(Tesla)c1; //Car 에서 Tesla로 형변환 했기에 자료형은 Tesla이고 변수로 지정해준 후 불러와도된다.
		
		System.out.println("변수사용 다운캐스팅 방법 : "+ t1.getBatteryCapacity());
	...
	public void ex4() {
		...
		Car c1 = new Tesla(); //현재 업캐스팅 된 상태!
	
		//Spark s1 = (Spark)c1;
		
		//실행 시 오류 
		//ClassCastException :형변환 예외
		//-> c1이 참조하는 객체는 Tasle인데 Spark 참조변수로 Tesla를 참조하려고 하면 문제 발생
		
		//해결방법 : instanceof 와 같이 사용한 후 파악해보면 좋다.
	...
	public void ex5() {
		...
		//프로그램 "실행전" 컴파일러는 getEngine() 메소드가 Car에 있는 걸로 인식해서 
		//c1.getEngine() 호출코드와 edu.kh.poly.ex1.model.vo.Car.getEngine() 메소드 코드를 연결해주는 것
		// => 정적 바인딩
		...
		//컴파일되었을 때와 실행했을때의 형태가 다른것 => 동적바인딩
		
		// 참조변수 c2가 Car타입으로 toString()도 Car의 toString() 호출 - 정적바인딩
		// 하지만 실행해보면 자식(Spark)의 toString()이 호출되는 것을 확인
		//-> 컴파일 시에는 부모(Car)와 바인딩 ==정적바인딩
		//-> "실행시"에는 자식 (Spark)의 오버라이딩 된 메소드와 바인딩 == 동적 바인딩
	}
}
