...
//		}
		...
		char[] arr = new char[input.length()]; //char배열 만드는데 문자열의 길이만큼 만들 것임
		...
		System.out.print("검색할 문자 입력 :"); //입력할 문자 띄우기
		char ch = sc.next().charAt(0); 

		int count = 0; //같은 글자 개수를 세기 위한 변수
		
		for(int i=0; i<arr.length; i++) {
			if(arr[i]== ch) { //ch는 == 사용가능
				count++;
				//arr[i] 값과 검색할 문자 ch가 같은 경우 카운트 늘려라!
			}
		}
		// 결과 출력
		if(count > 0) {
